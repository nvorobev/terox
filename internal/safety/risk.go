...
// предупреждений и подсветки, а НЕ доказанная граница безопасности. Настоящие границы:
...
const (
	...
	// затрагивает ВСЕ строки. Высший уровень риска.
...
// bool». Несёт уровень риска, признаки write/unqualified и
// человекочитаемые причины (какое слово/функция сработали) — чтобы UI объяснял отказ,
// а не просто блокировал.
type Decision struct {
	Level       RiskLevel
	Write       bool     // путь записи: нужен режим \write
	Unqualified bool     // безусловная запись: повышенный риск
...
// дают более богатую градацию для предупреждений и подсветки.
