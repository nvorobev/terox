...
записи и миграции включаются через `\write on`, а при настроенной `migration_role`
выполняются под ней.
...
- `make install` — собрать и поставить бинарь + `config.yaml` на PATH (без полной
  переустановки как у `make`; sudo только если каталог не writable). Перед заменой
  отличающегося пользовательского конфига сохраняет рядом резервную копию
  `config.yaml.bak.YYYYMMDD-HHMMSS`; одинаковый конфиг повторно не копируется.
...
| `prod` | `true` = production (красный бейдж + отдельные барьеры для незащищённых операций) |
...
```yaml
write_mode_default: false   # false: read-only; true: сразу включить режим записи
...
был пишущий запрос, повтор снова проверяет текущий режим записи, а не обходит `\write off`.
...
Начальное состояние задаётся `write_mode_default: false|true`. Режим записи — это
per-context lease: сбрасывается при смене хранилища/сервиса.

```
\write on                разрешить обычные записи без дополнительного подтверждения
\write off               снова запретить записи
...
- **Превью влияния:** по умолчанию выключено; включается через `\impact on`. После включения
  перед `UPDATE`/`DELETE` показывается, сколько строк затронет на каждом шарде.
  `UPDATE … FROM`/`DELETE … USING` превью не строят.
- `UPDATE`/`DELETE` без `WHERE` и `TRUNCATE` выполняются сразу, если режим записи включён.
- **`DROP DATABASE` запрещён безусловно** — отклоняется на любом пути (запись, `\migrate`,
  staged-rollout, `\i`), даже во включённом write-режиме: он необратимо уничтожает всю
...
**Первая запись с настройкой по умолчанию `write_mode_default: false`:**
```
\write on                            # включить режим записи (per-context lease)
update items set status='ok' where id=1;   # при \impact on: превью → выполнение → пер-шард статус
```
...
| Команда | Синтаксис | Назначение |
|---|---|---|
| `\h` (`\history`) | `\h` | история текущего сервиса (↑/↓ листать, Ctrl-R поиск); файлы `~/.config/terox/history-<service>` |
...
При включённом `\impact on` перед `UPDATE`/`DELETE` terox показывает, сколько строк
затронет на каждом шарде, а затем сразу выполняет запрос и показывает пер-шардовый статус:
